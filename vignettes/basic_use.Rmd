...
Once this is done,  it is possible to create a dummy crosstabs object.   The `data(classification)` step loads a `SpatialPointsDataFrame` called `loc_confuse`
```{r}
loc_confuse
```
This has a set of 98 ground-truthed points,  with predicted (via a support vector machine) and actual land cover classes. To create a `SpatialCrossTabs` object with a dummy crosstab at each observation point:
...
```
Here, around 5000 points are created on a hexagonal grid covering the bounding box of `loc_confuse` - these may be seen below:

```{r,fig.show='hold'}
par(mar=c(0,0,0,0)+0.1)
plot(loc_confuse,pch=16,col='navy',cex=1.5)
...
par(mar=c(0,0,0,0)+0.1)
plot(loc_confuse,pch=16,col='navy',cex=1.5)
plot(hg_port,pch=16,col='indianred',cex=0.6*hg_port$port,add=TRUE)
```
...
sg <- spsample(loc_confuse,5000,'stratified')
# Carry out the sampling
sg_port <- gwxtab_sample(sg,dummy_xtab,adapt(15),melt=pm)
# Plot the result
par(mar=c(0,0,0,0)+0.1)
plot(loc_confuse,pch=16,col='navy',cex=1.5)
plot(sg_port,pch=16,col='indianred',cex=0.5*sg_port$port,add=TRUE)
```
...
plot(hg_maj,col=find_col(hg_maj$choice,col_list),pch=16,cex=0.4)
plot(sg_maj,col=find_col(sg_maj$choice,col_list),pch=16,cex=0.4)
```
...
par(mar=c(0,0,0,0)+0.4)
plot(hg_rmaj,col=find_col(hg_rmaj$choice,col_list),pch=16,cex=0.4)
plot(sg_rmaj,col=find_col(sg_rmaj$choice,col_list),pch=16,cex=0.4)
```
...
par(mar=c(0,0,0,0)+0.4)
plot(hg_maj_ent,col=find_col(hg_maj_ent$choice,col_list),pch=16,cex=0.7*hg_maj_ent$ent)
plot(sg_maj_ent,col=find_col(sg_maj_ent$choice,col_list),pch=16,cex=0.7*sg_maj_ent$ent)
```
